...
func (wechatFileHelper *WechatFileHelper) Login() {
	GET(wechatFileHelper, "https://filehelper.weixin.qq.com/")
	//println(index)
		...
		if strings.Index(qrCode, "200") >= 0 {
			...
			for queryTime <= 25 { //二维码有效期20秒钟
				...
				if strings.Index(scanStatus, "408") >= 0 {
					...
					queryTime = 0
					for true && queryTime <= 10 {
						scanStatus, _ := GET(wechatFileHelper, "https://login.wx.qq.com/cgi-bin/mmwebwx-bin/login?loginicon=true&uuid="+qrcodeId+"&tip=1&r=582858735&appid=wx_webfilehelper")
						if strings.Index(scanStatus, "200") >= 0 {
							//说明手机确认登录成功
							//存放到userToken中。
							//ioutil.WriteFile("./userToeken", scanStatusBytes, 0777)
							println(scanStatus)
							redirectUri = findUrlRegex.FindString(scanStatus) + "&fun=new&version=v2"
							scaning = false
							break
						} else {
			...
			if !scaning {
				...
				//进行init请求的数据提取
				//主要包括SyncKey的提取
				marshal, err := json.Marshal(wechatFileHelper.GetBaseRequest())
				if err != nil {
					panic(err)
				}
...
func (wechatFileHelper *WechatFileHelper) ProcessMessage() bool {
	...
	if xSyncBody.AddMsgCount > 0 {
		for i := 0; i < xSyncBody.AddMsgCount; i++ {
			...
			case 1: //文本
				println("接收到文本：" + xSyncBody.AddMsgList[i].Content)

				break
			case 3: //img
				println("接收到图片：" + string(xSync.ImgWidth) + ":" + string(xSync.ImgHeight))
			...
			case 49: //file
				...
				ioutil.WriteFile("./"+xSync.FileName, downloadBytes, 0777)
				break
